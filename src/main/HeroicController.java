...
import static main.Tools2.calculateHeroicPaths;
import static main.Variable.*;

public class HeroicController implements Initializable {
    ...
    void AddAction(ActionEvent event) {
        ...
        if ( !isOnList ) {
            for ( Hero h: allheroes ) {
                if (h.getName().equals(currentRecord.getName())) {
                    currentRecord.setId(h.getId());
                    currentRecord.setRank(HERO_RANK);
                    break;
                }
            }
    ...
    public void initialize(URL location, ResourceBundle resources) {
        for (Hero h : hero ){
            records.add(new Record(h.getId(), HERO_RANK, h.getName(), h.getInventory()));
        }

        setCurrentRecord(null);

        for ( Hero h : heroicCombo ) {
    ...
    void CalculateClick(ActionEvent event) {
        ...
        new Thread(() -> {
            calculateHeroicPaths(integerProperty);
    ...
    private void setCurrentRecord(Record selectedRecord){
        if ( selectedRecord != null ) {
            ...
            if ( records.size() > 0 ) {
                ...
                currentRecord.setId(heroicCombo.get(0).getId());
                currentRecord.setRank(heroicCombo.get(0).getHeroClass());
                currentRecord.setName(heroicCombo.get(0).getName());
